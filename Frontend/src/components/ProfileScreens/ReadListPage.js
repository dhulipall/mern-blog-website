...
const ReadListPage = () => {
    ...
    useEffect(() => {
        const getUserReadingList = async () => {
            ...
            try {
                const { data } = await (await axios.get(API_URL+ `/user/readList`, config)).data
                setReadList(data)
                setLoading(false)
            }
            catch (error) {
            ...
            {loading ? <Loader /> :
                ...
                <div className="Inclusive-readList-page">
                    ...
                    <div className="readList-top-block">
                        ...
                        <img src={`https://mern-blog-website-dhsz.onrender.com/userPhotos/${activeUser.photo}`} alt={activeUser.username} />
